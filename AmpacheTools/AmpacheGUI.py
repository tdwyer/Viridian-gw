...
class AmpacheGUI:
	...
	def __init__(self, ampache_conn, audio_engine, db_session, is_first_time, version, wallet=''):
	...
	def main_gui_callback(self):
		...
		url      = self.db_session.variable_get('credentials_url')
                ...
                password = self.wallet.get_password(url, username)
	...
	def button_save_preferences_clicked(self, widget, data=None):
		...
		if self.ampache_conn.has_credentials: # credentials are NOT blank
			...
			self.db_session.variable_set('credentials_url', url)
                        # Save password with gpg-wallet
                        self.wallet.set_password(url, username, password)
                        # Set password to be saved in DB to gpg-wallet
                        self.db_session.variable_set('credentials_password', 'gpg-wallet')
